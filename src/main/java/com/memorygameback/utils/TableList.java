    ...
    public void print() {
        ...
        if (ucode) {
            for (int i = 0; i < rows; i++) {
                ...
                line.append(TLINE.repeat(Math.max(0, tableSizes[i] + 2 * spacing)));
            }
        ...
        for (int i = 0; i < rows; i++) {
            ...
            line.append(" ".repeat(Math.max(0, spacing)));
            line.append(part);
        }
        if (ucode) {
            line.append(gc(VERTICAL_TSEP));
        ...
        for (int i = 0; i < rows; i++) {
            ...
            line.append(String.valueOf(gc(BLINE)).repeat(Math.max(0, tableSizes[i] + 2 * spacing)));
        }
        if (ucode) {
